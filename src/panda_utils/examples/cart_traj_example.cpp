...
int main(int argc, char *argv[]) {
  ...
  // Get desired pose and command a short trajectory from the desired to the
  // current desired
  // {
  //   RCLCPP_INFO_STREAM(node->get_logger(),
  //                      "Getting current desired pose, press ENTER");
  //   std::cin.ignore();
  //
  //   rclcpp::wait_for_message<geometry_msgs::msg::PoseStamped>(
  //       current_desired_pose, node, "debug/desired_pose", 10s);
  // }

  // Calling action server for first trajectory
  // {
  //
  //   RCLCPP_INFO_STREAM(
  //       node->get_logger(),
  //       "Ready to call action server for the first trajectory, press ENTER");
  //   std::cin.ignore();
  //
  //   panda_interfaces::action::CartTraj_Goal cart_goal;
  //   cart_goal.initial_pose = current_desired_pose.pose;
  //   geometry_msgs::msg::Pose desired_pose;
  //   desired_pose = initial_pose.pose;
  //   cart_goal.total_time = 0.2;
  //   cart_goal.desired_pose = desired_pose;
  //   cart_goal.only_position = true;
  //
  //   RCLCPP_INFO_STREAM(node->get_logger(),
  //                      "Objective: Desired pose ["
  //                          << cart_goal.desired_pose.position.x << ", "
  //                          << cart_goal.desired_pose.position.y << ", "
  //                          << cart_goal.desired_pose.position.z
  //                          << "], Orientation (w, x, y, z): ["
  //                          << cart_goal.desired_pose.orientation.w << ", "
  //                          << cart_goal.desired_pose.orientation.x << ", "
  //                          << cart_goal.desired_pose.orientation.y << ", "
  //                          << cart_goal.desired_pose.orientation.z << "] in "
  //                          << cart_goal.total_time << "[s]");
