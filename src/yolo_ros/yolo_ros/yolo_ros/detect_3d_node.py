...
class Detect3DNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        detection_qos_profile = QoSProfile(
        ...
        # pubs
        self._pub = self.create_publisher(DetectionArray, "detections_3d", detection_qos_profile)
    ...
    def process_detections(
        ...
        if depth_image.dtype == np.float32:  # This corresponds to 32FC1
            self.get_logger().info(f"Depth image is {
                depth_image.dtype} (meters). Converting to 16UC1 (millimeters).", once=True)

            # a. Handle NaN values (Not-a-Number) which are common in float depth images.
            #    Replace them with 0, which we'll treat as an invalid depth reading.
            depth_image = np.nan_to_num(depth_image, nan=0.0)

            # b. Convert from meters (float) to millimeters (unsigned integer).
            #    We multiply by 1000 and then cast to a 16-bit unsigned integer.
